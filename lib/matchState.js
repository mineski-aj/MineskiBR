...
const DEFAULT = {
  ...
  // Mirrors whichever Today's Maps row `match` currently points at — kept
  // in sync automatically by html/match-dashboard.html's
  // updateMatchPickerMirror(), not editable directly. Downstream consumers
  // (e.g. Fullscreen.html's Consolidated Post fallback) read this as
  // "the current map" instead of looking up todayMaps themselves.
  map:      'Isolated',
  ...
  // Battle Royale redesign — an operator-set count (1-8, see BR_MAPS_MAX in
  // html/match-dashboard.html) of maps for the day/tomorrow, each with its
  // own time, map name (fixed BR_MAP_NAMES pool), and a Done/not-done flag.
  // Today and tomorrow are independent lists/counts — tomorrow's is usually
  // still being planned while today's is live.
  todayMapsCount: 5,
  todayMaps: [
    { time: '', mapName: 'Isolated', done: false },
    { time: '', mapName: 'Isolated', done: false },
    { time: '', mapName: 'Isolated', done: false },
    { time: '', mapName: 'Isolated', done: false },
    { time: '', mapName: 'Isolated', done: false },
