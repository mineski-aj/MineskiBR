...
// Bank registry persistence — the browser UI (html/ss-animator.html) is
// the only thing that ever DEFINES banks (it keeps its own copy in
// localStorage and pushes it here via 'register_banks' on load), but the
// HTTP trigger routes below (for a StreamDeck/switcher macro) need
// bankRegistry to already be populated with NO browser involved — that's
// the whole point of a hardware trigger. Without this file, bankRegistry
// started as { ss1: [], ss2: [] } on every server restart and stayed
// that way — silently failing every HTTP trigger with "No banks
// registered" — until some browser happened to load the dashboard's SS
// Animator tab again. Mirroring every 'register_banks' message to disk
// and reloading it here on startup means triggers work immediately after
// a restart, with zero browser dependency.
const BANKS_FILE = path.join(__dirname, '..', 'ss_animator_banks.json');

function readBanksFile() {
  try {
    const d = JSON.parse(fs.readFileSync(BANKS_FILE, 'utf8'));
    return { ss1: Array.isArray(d.ss1) ? d.ss1 : [], ss2: Array.isArray(d.ss2) ? d.ss2 : [] };
  } catch (e) {
    return { ss1: [], ss2: [] };
  }
}

function writeBanksFile(banks) {
  try { fs.writeFileSync(BANKS_FILE, JSON.stringify(banks, null, 2)); }
  catch (e) { console.error('[SSANIM] failed to persist banks:', e.message); }
}
...
// seeded here from disk (see readBanksFile()/writeBanksFile() above) so
// it isn't empty on a fresh server start with no browser connected.
// Structure: { ss1: [ {name, duration, bg, boxes[]}, ... ], ss2: [...] }
let bankRegistry = readBanksFile();
  ...
  switch (msg.cmd) {
    ...
    case 'set_ssrc': {
      ...
      (msg.boxes || []).forEach((b, i) => {
        ...
        if (b.cropped !== undefined) p.cropped = b.cropped;
        if (b.cropTop !== undefined) p.cropTop = Math.round(b.cropTop);
        if (b.cropBottom !== undefined) p.cropBottom = Math.round(b.cropBottom);
        if (b.cropLeft !== undefined) p.cropLeft = Math.round(b.cropLeft);
        if (b.cropRight !== undefined) p.cropRight = Math.round(b.cropRight);
    ...
    case 'set_ssrc_box_source': {
      ...
      if (!requireATEM(ws)) return;
      const p = { source: msg.source, enabled: msg.enabled };
      if (msg.cropped !== undefined) p.cropped = msg.cropped;
      if (msg.cropTop !== undefined) p.cropTop = Math.round(msg.cropTop);
      if (msg.cropBottom !== undefined) p.cropBottom = Math.round(msg.cropBottom);
      if (msg.cropLeft !== undefined) p.cropLeft = Math.round(msg.cropLeft);
      if (msg.cropRight !== undefined) p.cropRight = Math.round(msg.cropRight);
      safe(() => atem.setSuperSourceBoxSettings(p, boxId, ssrcId));
      break;
    }
...
function runAnimation(ssKey, ssrcId, from, bank) {
  ...
  // immediately rather than eased over the x/y/size loop below.
